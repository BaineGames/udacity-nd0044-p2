...
class TriviaTestCase(unittest.TestCase):
    ...
    # test getting questions in a specific category
    def test_get_questions_from_category(self):
        res = self.client().get(
            "/api/categories/{}/questions".format(self.category_id_for_testing))
        self.assertEqual(res.status_code, 200)
        response = json.loads(res.data)
        self.assertTrue(response['success'])
    ...
    def test_add_question(self):
        res = self.client().post("/api/questions",
                                 json=self.add_new_question_data)  # test adding a question
        self.assertEqual(res.status_code, 200)
        response = json.loads(res.data)
        self.assertTrue(response["success"])
        pass
    ...
    def test_delete_question(self):
        get_last_question = Question.query.order_by(
            Question.id.desc()).limit(1).all()
        question_id = get_last_question[0].id
        res = self.client().delete(
            "/api/questions/{}".format(question_id))  # test deleting a question
        self.assertEqual(res.status_code, 200)
        response = json.loads(res.data)
        self.assertTrue(response["success"])
        pass
    ...
    def test_search_questions(self):
        res = self.client().post(
            ...
            json=self.search_term)  # test searching
        self.assertEqual(res.status_code, 200)
        response = json.loads(res.data)
        self.assertTrue(response["success"])
        self.assertGreater(len(response['questions']), 0)
    ...
    def test_400(self):
        res = self.client().post("/api/quizzes", content_type='application/json',
                                 data="abc")  # test malformed data to an endpoint
